...
3. Download and run:
   ...
   - Linux x64: `*-linux-x86_64.tar.gz` — extract and run `MTGA Constructed Testing/MTGA Constructed Testing`. A current desktop (libEGL / X11 or Wayland) is required. The client looks for `Player.log` under Steam Proton (including Flatpak/Snap), Wine, Lutris, and Bottles; Advanced can still override.
